...
<script>
document.querySelector('.btnAddToCart').addEventListener('click', function(event) {
    event.preventDefault();  // Prevent the form submission

    // Get selected beanie type
    const beanieType = document.querySelector('input[name="beanie"]:checked') ? document.querySelector('input[name="beanie"]:checked').value : null;
    
    // Get selected color
    const color = document.getElementById('beanie-color').value;

    // Get selected printing option
    const selectedPrinting = document.querySelector('.printing-option.active');
    const printingId = selectedPrinting ? selectedPrinting.dataset.id : null;
    
    // Get quantities and prices (from your dynamic data)
    const quantity = document.getElementById("quantity-input").value;
    const printingPrice = calculatePrice(quantity, selectedPrintingQuantities, pricesForSelectedPrinting); // Calculate printing price
    const productPrice = calculatePrice(quantity, quantities, prices); // Calculate product price
    const deliveryPrice = calculatePrice(quantity, quantitiesDelivery, pricesDelivery); // Calculate delivery price

    // Send data to the server via AJAX (using fetch here)
    fetch('/cart/add', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            product_id: document.querySelector('input[name="product_id"]').value, // Use the product_id from the hidden input
            color_id: color, // Color selected
            quantity: quantity, // Quantity entered
            beaniestype: beanieType === 'beanie1' ? 1 : 2, // Beanie type (1 or 2)
            printing_id: printingId, // Printing option selected
            printing_price: printingPrice, // Printing price
            product_price: productPrice, // Product price
            delivery_price: deliveryPrice // Delivery price
        })
    })
