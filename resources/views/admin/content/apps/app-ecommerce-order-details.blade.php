      ...
      <div class="col-md-4 col-lg-6">
        <div class="card mb-4 card-equal-height">
          ...
          <div class="card-body">
            ...
            <p class="mb-0"><Span style="font-weight: bold">Firstname:</Span> {{$order->ShippingDetails->firstname}}</p>


            <p class="mb-0"><Span style="font-weight: bold">Lastname:</Span> {{$order->ShippingDetails->lastname}}</p>
            <p class="mb-0"><Span style="font-weight: bold">Company Name:</Span> {{$order->ShippingDetails->companyname}}</p>
            <p class="mb-0"><Span style="font-weight: bold">Email:</Span> {{$order->ShippingDetails->email}}</p>
            <p class="mb-0"><Span style="font-weight: bold">Phone:</Span> {{$order->ShippingDetails->phone}}</p>
            <p class="mb-0"><Span style="font-weight: bold">Address:</Span> {{$order->ShippingDetails->address}}</p>
            <p class="mb-0"><Span style="font-weight: bold">Additional Info:</Span> {{$order->ShippingDetails->additional_info}}</p>
          </div>
        </div>
      </div>
      ...
      @foreach ($order->Order_files as $item)
          ...
          <div class="mb-2 d-flex align-items-center"> <!-- Use d-flex to align items horizontally -->
              ...
              @elseif(in_array($fileExtension, ['pdf']))
                  ...
                  <a href="{{ asset($item->file) }}" class="fs-12 fw-500 color-primary" target="_blank">View PDF</a>
              ...
              <div class="ms-3"> <!-- Add margin-left for spacing between the image and text -->
                  <h4>{{ $item->title }}</h4>
                  <a href="{{ asset($item->file) }}" class="fs-12 fw-500 color-primary" download>Download</a>
              </div>
          </div>
      @endforeach
...
<div class="modal fade" id="fileUploadModal" tabindex="-1" aria-labelledby="fileUploadModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="{{ route('order.file.upload', $order->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
